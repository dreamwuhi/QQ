...
namespace DuiLib
{
	...
	CFriendsUI::CFriendsUI(CPaintManagerUI& paint_manager)
		...
		, delay_number_(0)
		, delay_left_(0)
		, level_expand_image_(_T("<i login_skin/list_icon_b.png><x 10>"))		//列表展开与关闭的图标
		, level_collapse_image_(_T("<i login_skin/list_icon_a.png><x 10>"))
	...
	bool CFriendsUI::Add(CControlUI* pControl)			//添加控件
	{
		if (!pControl)
			return false;
	...
	bool CFriendsUI::AddAt(CControlUI* pControl, int iIndex)   //添加控件
	{
		if (!pControl)
			return false;
	...
	bool CFriendsUI::Remove(CControlUI* pControl)  //删除控件
	{
		if (!pControl)
			return false;
	...
	bool CFriendsUI::RemoveAt(int iIndex)				//删除控件
	{
	...
	Node* CFriendsUI::AddNode(const FriendListItemInfo& item, Node* parent)
	{
		...
		if (item.folder)
			node->data().has_child_ = !item.empty; //有孩子 （根节点）
		else
			node->data().has_child_ = false;			//子节点 没有孩子

		node->data().folder_ = item.folder;			//是否为根

		node->data().child_visible_ = (node->data().level_ == 0);
		node->data().child_visible_ = false;		//孩子不可见

		node->data().text_ = item.nick_name;		
		node->data().value = item.id;				//貌似 没有用处
		node->data().list_elment_ = pListElement;		//关联列表控件

		if (!parent->data().child_visible_)		//设置孩子的可见性                //根节点设置
			pListElement->SetVisible(false);	//最终体现在列表控件上

		if (parent != root_node_ && !parent->data().list_elment_->IsVisible()) //子节点设置
			pListElement->SetVisible(false);

		CDuiRect rcPadding = text_padding_;				//设置列表控件起始边距
		...
		CButtonUI* log_button = static_cast<CButtonUI*>(paint_manager_.FindSubControlByName(pListElement, kLogoButtonControlName));//找图标按钮
		if (log_button != NULL)
		{
			if (!item.folder && !item.logo.IsEmpty())   //设置子节点
			{
#if defined(UNDER_WINCE)
				_stprintf(szBuf, _T("%s"), item.logo);
#else
				_stprintf_s(szBuf, MAX_PATH - 1, _T("%s"), item.logo);		//szBuf中存 logo的路径
#endif
			...
			else     //设置根节点
			{
				...
				if (logo_container != NULL)			//根节点，没有logo，则不显示
		...
		if (node->data().has_child_)			//只有根节点有孩子节点
		{
			if (node->data().child_visible_)			//根据孩子节点是否可见 设置展开箭头 还是 关闭箭头
		...
		if (!item.folder && !item.description.IsEmpty()) //设置子节点的描述 资源
		{
...
#else
				_stprintf_s(szBuf, MAX_PATH - 1, _T("<c #808080>%s</c>"), item.description);
#endif
				description->SetShowHtml(true);
				description->SetText(szBuf);
				TCHAR szSign[MAX_PATH]={0};
				_stprintf_s(szSign,MAX_PATH-1,L"个性签名更新:%s",item.description);
				description->SetToolTip(szSign);
			}
		}

		if(item.folder)
		{
			pListElement->SetFixedHeight(31);
		}
		else
		{
			pListElement->SetFixedHeight(kFriendListItemSelectedHeight);
		}
	...
	{
		if (!node || node == root_node_) //节点为空 或者是 头结点 直接返回
			return;

		if (node->data().child_visible_ == visible) //是否可见属性 没改变 直接返回
			return;

		node->data().child_visible_ = visible;	//设置为当前属性

		TCHAR szBuf[MAX_PATH] = {0};
		CDuiString html_text;
		if (node->data().has_child_)		//对 根节点
		{
	...
	bool CFriendsUI::SelectItem(int iIndex, bool bTakeFocus)   //列表行 根据 是否是当前选中行 显示不同的状态
	{
